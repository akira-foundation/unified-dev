...
#[tauri::command]
pub async fn set_local_repository_remote(
    repo_id: String,
    remote_url: String,
    state: State<'_, AppState>,
) -> AppResult<()> {
    repos::set_remote(repo_id, remote_url, &state.pool().await?).await
}

#[tauri::command]
pub async fn link_local_repository_to_organization(
    repo_id: String,
    organization_id: String,
    state: State<'_, AppState>,
) -> AppResult<()> {
    repos::link_organization(repo_id, organization_id, &state.pool().await?).await
}
