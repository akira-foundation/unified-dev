...
#[tauri::command]
pub async fn send_test_notifications(app: AppHandle) -> AppResult<()> {
    use crate::app::notifications::{
        notify, NotificationCategory, NotificationInput, NotificationSeverity,
    };

    let samples = [
        (
            NotificationCategory::Autopilot,
            NotificationSeverity::Success,
            "Autopilot job completed",
            "12 issues processed successfully",
        ),
        (
            NotificationCategory::Sync,
            NotificationSeverity::Warning,
            "Sync failed: pull requests",
            "akira-io/billing: rate limit exceeded, retrying in 60s",
        ),
        (
            NotificationCategory::Update,
            NotificationSeverity::Info,
            "Gemini CLI update available",
            "0.29.7 → 0.38.1 (run: brew upgrade gemini-cli)",
        ),
        (
            NotificationCategory::UsageLimit,
            NotificationSeverity::Warning,
            "Approaching daily run limit",
            "Last run of the day. 4/5 used.",
        ),
        (
    ...
    for (category, severity, title, body) in samples {
        let _ = notify(
            &app,
            NotificationInput {
                category,
