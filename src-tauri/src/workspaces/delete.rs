...
pub async fn delete_local_repository(repo_id: String, pool: &sqlx::SqlitePool) -> AppResult<()> {
    ...
    if let Some(row) = repo_record {
        let workspace_root: String = row.try_get("workspace_root").map_err(AppError::Database)?;
