...
pub async fn add_thread(repo_id: String, pool: &sqlx::SqlitePool) -> AppResult<ThreadConfig> {
    let row = sqlx::query_as::<_, (String, String)>(
        "SELECT workspace_root, name FROM local_repositories WHERE id = ?",
    )
    .bind(&repo_id)
    .fetch_optional(pool)
    .await?;

    let (workspace_root, _name) = row.ok_or_else(|| {
    ...
    create_initial_thread(repo_id, &base_repo_path, &workspace_root, pool).await
}
