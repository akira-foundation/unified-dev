...
pub async fn resolve_context(
    ...
    let (repo_name, source_path, remote_url, workspace_root) =
        row.ok_or_else(|| AppError::Internal(format!("Repository '{}' not found", repo_id)))?;
    ...
    let remote_candidate = remote_url
        ...
        .or_else(|| {
            crate::app::repos::git::get_remote_url(std::path::Path::new(&source_path), "origin")
        })
        .unwrap_or(source_path);

    let (owner, normalized_repo_name) =
        parse_github_remote(&remote_candidate).ok_or_else(|| {
...
pub async fn list_issues(repo_id: String, state: State<'_, AppState>) -> AppResult<Vec<IssueDto>> {
    let context = resolve_context(&repo_id, &state).await?;
    crate::app::issues::list(
        state,
        context.organization_id,
        context.repo_name,
        Some("all_open".to_string()),
        None,
    )
    .await
...
pub async fn list_pull_requests(
    ...
    let context = resolve_context(&repo_id, &state).await?;
    crate::app::orgs::pull_requests::list(
        state,
        context.organization_id,
        context.repo_name,
        None,
