...
pub async fn delete_local(
    ...
    if let Some(row) = repo_record {
        let workspace_root: String = row.try_get("workspace_root").map_err(crate::app::support::error::AppError::Database)?;
        let workspace_path = Path::new(&workspace_root);
        if workspace_path.exists() {
            std::fs::remove_dir_all(workspace_path).map_err(crate::app::support::error::AppError::Io)?;
        }
    }
