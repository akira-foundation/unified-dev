...
use crate::app::repositories::types::{AddLocalRepositoryResponse, LocalRepository};
...
pub async fn add_remote(
    ...
    let repo_name = git::repo_name_from_url(&url).ok_or_else(|| crate::support::error::AppError::Internal("Could not parse repository name from URL.".to_string()))?;
    let home_dir = dirs::home_dir().ok_or_else(|| crate::support::error::AppError::Internal("Could not find home directory".to_string()))?;
    let workspace_root = home_dir.join(".unifieddev").join("workspaces").join(&repo_name);
    if !workspace_root.exists() {
        std::fs::create_dir_all(&workspace_root).map_err(crate::support::error::AppError::Io)?;
    }
    ...
    let existing: Option<(String,)> = sqlx::query_as("SELECT id FROM local_repositories WHERE source_path = ? LIMIT 1")
        ...
        .await?;
    if existing.is_some() {
        return Err(crate::support::error::AppError::Internal(format!("The repository '{}' has already been added.", repo_name)));
    }
